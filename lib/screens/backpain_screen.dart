...
class BackpainScreen extends StatefulWidget {
  const BackpainScreen({super.key});


  @override
  _BackpainScreenState createState() => _BackpainScreenState();
}

class _BackpainScreenState extends State<BackpainScreen> {
  final String searchKeyword = "backpain"; // Sabit arama terimi
  ...
  Future<void> fetchVideos({bool loadMore = false}) async {
    ...
    try {
      ...
      List<SearchItems> filteredResults = (result.searchItems ?? []).where((video) {
        ...
        return title.contains("backpain") || description.contains("backpain"); 
      }).toList();
